...
class DM_Serial:
    ...
    def get_all_latest(self) -> Tuple[Dict[int, Tuple[float, float, float]], float, int]:
        """线程安全地获取所有 RID 的最新数据 {rid: (f1,f2,f3)}。"""
        with self._latest_lock:
            return dict(self._latest_by_rid), self._latest_ts, self._latest_count
        ...
        try:
            while evt and not evt.is_set():
                ...
                if frames:
                    now = time.time()
                    with self._latest_lock:
                        for rid, vals in frames:
                            self._latest_by_rid[rid] = vals
                        self._latest_pkt = frames[-1]
                        self._latest_ts = now
